      ...
      <div className="page-container py-8 px-6">
        <div className="flex items-center justify-between mb-6">
          <div>
            <h1 className="text-3xl font-bold text-slate-800 dark:text-slate-200 mb-2">
              {t('employerNotifications.title')}
            </h1>
            <p className="text-slate-600 dark:text-slate-400">{t('employerNotifications.subtitle')}</p>
          </div>
          {unreadCount > 0 && (
            <button
              onClick={handleMarkAllRead}
              disabled={markingRead}
              className="px-4 py-2 bg-brandOrange text-white rounded-lg font-medium hover:bg-brandOrange/90 transition-colors disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-2"
            >
              <CheckCircle2 className="w-4 h-4" />
              {t('employerNotifications.markAllRead')}
            </button>
          )}
        </div>
        ...
        {loading ? (
          ...
          <div className="text-center py-12 bg-white dark:bg-slate-800 rounded-xl shadow-md border border-slate-200 dark:border-slate-700">
            ...
            <p className="text-slate-600 dark:text-slate-400">{t('employerNotifications.noNotifications')}</p>
          </div>
        ) : (
              ...
              <div
                ...
                className={`
                  ...
                  ${notification.read
                    ...
                    : 'border-brandOrange/50 dark:border-brandOrange/30 bg-orange-50/50 dark:bg-orange-900/10'
                  }
                `}
              >
                <div className="flex items-start gap-4">
                  <div className={`
                    ...
                    ${notification.read
                      ...
                      : 'bg-brandOrange/10 dark:bg-brandOrange/20'
                    }
                  `}>
                    <Bell className={`
                      w-5 h-5
                  ...
                  <div className="flex-1">
                    <div className="flex items-start justify-between mb-2">
                      ...
                      {!notification.read && (
                        <span className="w-2 h-2 bg-brandOrange rounded-full flex-shrink-0 mt-2"></span>
                      )}
                    </div>
